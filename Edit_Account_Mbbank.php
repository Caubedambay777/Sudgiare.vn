                        ...
                        <table class="table table-bordered table-striped fs--1 mb-3 dataTables-NTD dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                            ...
                            <tbody>
                                ...
                                if ($url_site_name == $URL_ADMIN) {
                                    $getlistcard = $LTT->get_list("SELECT * FROM `mbbankauto`  ORDER BY `id` DESC ");
                                } else {
                                    $getlistcard = $LTT->get_list("SELECT * FROM `mbbankauto` WHERE  `sitename` = '" . $url_site_name . "' ORDER BY `id` DESC ");
                                }
                                    ...
                                    foreach ($getlistcard as $rowucard) { ?>
                                        <tr class="odd">
                                            ...
                                            <td>
                                                ...
                                                <form submit-ajax="NTD" action="<?= BASE_URL('private/mbbank/balance') ?>" method="post" api_token="<?= $LTT->getUsers('token'); ?>">
                                                    <input type="hidden" name="id" value="<?= $rowucard['id'] ?>">
